...
import { ContactType, ErrorReportDetails, DeviceType, PCOSType, SPOSType, SmartphoneType, BrowserType } from "@/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("Received contact data:", JSON.stringify({ type, title, content: content?.substring(0, 100), errorReportDetails }, null, 2));
    }
    ...
    if (clientEmail && privateKey && getApps().length > 0) {
      ...
      try {
        ...
        if (existingApps.length > 0) {
          console.log("Reinitializing Firebase Admin with credentials (existing app found)");
    ...
    console.log("Skipping connection test, proceeding directly to user lookup");
    ...
    if (type === "error" && errorReportDetails) {
      try {
        console.log("Processing errorReportDetails:", JSON.stringify(errorReportDetails, null, 2));
        ...
        console.log("Validated errorReportDetails:", JSON.stringify(validatedErrorDetails, null, 2));
    ...
    try {
      ...
      console.log("Saving contact data to Firestore:", JSON.stringify(debugData, null, 2));
      
      // contactDataの構造を確認
      console.log("contactData type:", typeof contactData);
      console.log("contactData.createdAt type:", typeof contactData.createdAt);
      console.log("contactData.createdAt instanceof Timestamp:", contactData.createdAt instanceof Timestamp);
    ...
    try {
      ...
      console.log("Calling GitHub issue creation function:", createIssueUrl);
      ...
      console.log("GitHub issue creation response status:", issueResponse.status);
      ...
      if (issueResponse.ok) {
        ...
        if (issueData.url) {
          ...
          console.log("GitHub issue URL saved to Firestore");
        }
      } else {
