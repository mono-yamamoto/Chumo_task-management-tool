...
Always use `run_in_background: true` when running difit via Bash tool. This avoids timeout issues since difit starts a local server and waits for the user to finish reviewing in the browser.

1. Run `difit` with `run_in_background: true`
2. The browser opens and the user reviews at their own pace
3. When the background task completes, read the output file to retrieve review comments
4. If review comments are found, address them. If none, treat as "no comments."
